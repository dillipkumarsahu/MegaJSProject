...
function show_profile_pic(){
		...
		pic_box.style.backgroundImage="url("+image_name+")";
...
// unit of measure ani,ation coding ....
function unit_of_measure(){
	var unit_btn = document.getElementById("unit-of-measure");
	var p_btn = document.getElementById("primary-content");
	var s_btn = document.getElementById("secondary-content");
	var cross = document.getElementById("close_box");
	...
	unit_btn.onclick=function(){
		...
		setTimeout(function(){
			...
			s_btn.style.transform="rotateX(-180deg)";
			s_btn.style.webkitTransform="rotateX(-180deg)";
	    ...
	    form.onsubmit=function(){
	    	...
	    	var unit_object = {symbol:symbol,formal_name:formal};
...
function sales_voucher(){
	...
	sales_btn.onclick = function(){
		...
		for (var i = 0; i < localStorage.length; i++) {
			...
			{
				var tax_item = localStorage.getItem(all_keys);
...
function voucher_logo_details(){
	var voucher_logo = document.getElementById("vou-logo");
...
//adding item
var store_subtotal,store_tax = [],store_total,store_dues,store_paid,all_voucher_no = 1;
function add_item(){
	var product_table = document.getElementById("product-table");
	var tr = document.createElement("TR");
	var td_item = document.createElement("TD");
	td_item.style.width="30%";
	td_item.style.border="none";
	var td_price = document.createElement("TD");
	td_price.style.width="18%";
	td_price.style.border="none";
	var td_qty = document.createElement("TD");
	td_qty.style.width="15%";
	td_qty.style.border="none";
	var td_unit = document.createElement("TD");
	td_unit.style.width="10%";
	td_unit.style.border="none";
	var td_amount = document.createElement("TD");
	td_amount.style.width="20%";
	td_amount.style.border="none";
	var td_del = document.createElement("TD");
	td_del.style.width="5%";
	td_del.style.border="none";
	var input_item = document.createElement("INPUT");
	input_item.className="item";
	input_item.type="text";
	input_item.placeholder="Item Desciption";
	input_item.style.width="100%";
	input_item.style.padding="10px";
	input_item.style.borderRadius="4px";
	input_item.style.border="1px solid #d4d4d4";
	input_item.style.marginTop="2px";
	...
	input_price.style.padding="10px";
	input_price.style.borderRadius="4px";
	input_price.style.border="1px solid #d4d4d4";
	input_price.style.marginTop="2px";
	input_price.style.textAlign="center";
	...
	input_qty.style.width="100%";
	input_qty.style.padding="10px";
	input_qty.style.borderRadius="4px";
	input_qty.style.border="1px solid #d4d4d4";
	input_qty.style.marginTop="2px";
	input_qty.style.textAlign="center";
	input_qty.disabled=true;
	var input_unit = document.createElement("SELECT");
	input_unit.className="unit";
	input_unit.style.width="100%";
	input_unit.style.padding="10px";
	input_unit.style.borderRadius="4px";
	input_unit.style.border="1px solid #d4d4d4";
	input_unit.style.marginTop="2px";
	input_unit.style.textAlign="center";
		for (var i = 0; i < localStorage.length; i++) {
			var all_keys = localStorage.key(i);
	...
	input_amount.style.padding="10px";
	input_amount.style.borderRadius="4px";
	input_amount.style.border="1px solid #d4d4d4";
	input_amount.style.marginTop="2px";
	input_amount.style.textAlign="center";
	input_amount.className="amount";
	var i_del = document.createElement("I");
	i_del.className="fa fa-trash";
	i_del.style.cursor="pointer";
	i_del.style.fontSize="20px";
	...
	td_del.append(i_del);
	i_del.onclick=function(){
		var deletTd = this.parentElement;
		var deletTr = deletTd.parentElement;
		deletTr.remove();
	}
...
function current_date(){
	...
	date_td.innerHTML += current_date+"-"+Number(current_month+1)+"-"+current_year;
	store_date = current_date+"-"+Number(current_month+1)+"-"+current_year;
}
current_date();
...
function tax_btn(){
	...
	var btn_link = document.getElementById("tax-link");
	btn_link.onclick=function(){
		if (btn.offsetHeight == 60) {
...
function getbill(){
		...
		var print_btn = document.getElementById("print-btn");
		var voucher = document.getElementById("sales-voucher");
		print_btn.onclick=function(){
		var remove = document.getElementsByClassName("hide");
		var color = document.getElementsByClassName("color-print");
		
		var i,store_item = [],store_price = [],store_qty = [],store_amount = [],store_unit = [];
	...
	for (var i = 0; i < select.length; i++) {
		...
		select[i].style.border="none";
			...
			voucher.style.top="-250px";
		document.getElementById("voucher-close").style.display="none";
		document.getElementById("add-item").style.display="none";
		document.getElementById("cal-table").border = "1";
		var print_btn_td = this.parentElement;
		var print_btn_tr = print_btn_td.parentElement;
		print_btn_tr.remove();
		var input = document.getElementsByTagName("INPUT");
			...
			for (var i = 0; i < localStorage.length; i++) 
			{
				...
				if (all_keys == user_input) 
				{
					...
					var buyer_extract = JSON.parse(buyer_string);
						document.getElementById("sales-btn").onclick();
						document.getElementById("voucher-no").innerHTML= all_keys;
				...
				var amount_input = document.getElementsByClassName("amount");
				var previous_amount = 0;
				for (var j = 0; j <  amount_input.length; j++) {
				...
				var reserve = 0;
					...
					document.getElementById("Date").innerHTML += "Billing date : "+buyer_extract.store_date; 
...
function manage_tax(){
	...
	//open tax detail in tax setup box
	manage_tax.onchange = function(){
		...
		tax_icon.onclick = function(){
			var confirm	= window.confirm("Are you sure to delete this tax ?");
			if (confirm == true) {
				localStorage.removeItem(reserve);
				window.location = location.href;
			}
		}
		var tax_details = localStorage.getItem(this.value);
		var tax_extract = JSON.parse(tax_details);
...
//shurt company
function shurt_company(){
	var shurt_btn = document.getElementById("shurt-company");
	shurt_btn.onclick=function()
	{
